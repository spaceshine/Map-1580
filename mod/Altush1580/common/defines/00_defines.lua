...
NGame = {
	START_DATE = "2022.9.1.9",
	END_DATE = "2025.1.1.1",
	MAP_SCALE_PIXEL_TO_KM = 0.002,					-- Yes, we did the math
	...
	GAME_SPEED_SECONDS = { 4.0, 1.0, 0.4, 0.2, 0.0 }, -- game speeds for each level. Must be 5 entries with last one 0 for unbound
	MAJOR_PARTICIPANTS_FOR_MAJOR_WAR = 1,			-- Minimum number of major countries involved in a war to consider it major enough to not end the game even though the enddate has been reached.
	COMBAT_LOG_MAX_MONTHS = 12,
	MESSAGE_TIMEOUT_DAYS = 6,						-- Useful if running the handsoff game. The popup messages that doesn't require the player respond will automatically hide after some timeout.
	AIR_LOG_TIMEOUT_HOURS = 2,						-- Data storring data
	EVENT_TIMEOUT_DEFAULT = 1,						-- Default days before an event times out if not scripted
	MISSION_REMOVE_FROM_INTERFACE_DEFAULT = 1,		-- Default days before a mission is removed from the interface after having failed or completed
	DECISION_ALERT_TIMEOUT_DAYS = 5,				-- Days left when player will be alerted about timing out events or decisions
...
NDiplomacy = {
	...
	BASE_TRUCE_PERIOD = 18,						-- Base truce period in days.
	TRUCE_PERIOD_AFTER_KICKING_FROM_FACTION = 6,				-- Truce period after kicking someone from faction in days.
	NUM_DAYS_TO_ENABLE_KICKING_NEW_MEMBERS_OF_FACTION = 9,			-- Number of days before being able to kick a new member of faction
	NUM_DAYS_TO_ENABLE_REINVITE_KICKED_NATIONS = 9,			-- Number of days before being able to re invite a kicked nation to your faction
	...
	TRUCE_BREAK_COST_PP = 600,						-- Base cost in PP of breaking a truce by joining a war or accepting a call to war ( you cannot declare war yourself until the truce if up ), this is then multiplied by the time left on the truce ( so once half the truce is up it only cost 50% of this )
	...
	NAP_EXPIRY_MONTHS = 4,                         -- NAPs expire after this many months
	NAP_UNBREAKABLE_MONTHS = 1,                    -- NAPS cannot be broken for this many months
	NAP_FORCE_BALANCE_RULE_MONTHS = 1,        		-- The NAP border force balance rule changes with this interval
	...
	VOLUNTEERS_PER_COUNTRY_ARMY = 0.5,				-- Each army unit owned by the source country contributes this amount of volunteers to the limit.
	...
	VOLUNTEERS_DIVISIONS_REQUIRED = 2,				-- This many divisons are required for the country to be able to send volunteers.
	...
	TENSION_TIME_SCALE_START_DATE = "2022.9.1.9",	-- Starting at this date, the tension values will be scaled down (will be equal to 1 before that)
	TENSION_TIME_SCALE_MONTHLY_FACTOR = -0.5,		-- Timed tension scale will be modified by this amount starting with TENSION_TIME_SCALE_START_DATE
	...
	REVOKE_GUARANTEE_COST = 0,
	BASE_CONDITIONAL_PEACE_WARESCORE_RATIO = 1.0, 	-- Warscore ratio needed for the losing side to able to surrender.
	...
	OPINION_FOR_DEMO_FROM_WT_GENERATION = 0.0,		-- How much less do democracies like us if we generate world tension
	...
	ASSUME_FACTION_LEADERSHIP_MIN_MANPOWER_RATIO = 1.25,		-- The minimum ratio of manpower that a country must have compared to the current leader in order to assume leadership.
	ASSUME_FACTION_LEADERSHIP_MIN_FACTORY_RATIO = 1.0,		-- The minimum ratio of factories that a country must have compared to the current leader in order to assume leadership.
	ASSUME_FACTION_LEADERSHIP_COOLDOWN_DAYS = 20,			-- Number of days after formation of faction or change in leadership before another country is allowed to assume leadership.
	ASSUME_FACTION_SPYMASTER_COOLDOWN_DAYS = 999,			-- Number of days after change of Spy Master before another country is allowed to become Spy Master.
...
NCountry = {
	...
	BASE_RESEARCH_SLOTS = 5,						-- Base number of research slots per country.
	POPULATION_YEARLY_GROWTH_BASE = 0.000,			-- basic population growth per year, used for monthly manpower gain
	...
	RESISTANCE_DECAY_WHEN_NO_GROWTH = 0.00,		-- Resistance will fall by this much each day if there is nothing increasing it ( no VPs and no spread from neighbors )
	...
	NUCLEAR_PRODUCTION_SCALE = 10,					-- +1 nuclear_production gives 1 nuke per year
	...
	MAX_NON_CORE_MANPOWER_FACTOR = 0.2,				-- max clamp for recruitable local non core manpower factor for states
	...
	MIN_NAVAL_SUPPLY_EFFICIENCY = 0.0,				-- Min ratio when supplies will be considered delivered from the capital by naval path
	PARADROP_AIR_SUPERIORITY_RATIO = 1.5,			-- Min ratio of air superiority for paradropping
	...
	SURRENDER_LIMIT_MULT_FOR_COUNTRIES_WITH_NO_CORES = 1.0, -- Countries with no owned cores will their surrender level multiplied by this amount
	...
	BOMBING_WAR_SUPPORT_PENALTY_SCALE = -0.015, 			-- Scaling of bomber damage to war support impact, will be added weekly as a war support penalty
	MAX_BOMBING_WEEKLY_WAR_SUPPORT_PENALTY = -2.0,		-- Max penalty that will gained per week from bomber's damage
	BOMBING_WEEKLY_WAR_SUPPORT_PENALTY_DECAY = 0.5,		-- Weekly decay of bomber damage war support penalty
	MAX_BOMBING_WAR_SUPPORT_IMPACT = -1.0,					-- Max total penalty from bomber's damage

	HEROES_BEING_KILLED_WAR_SUPPORT_PENALTY_SCALE = -0.00,			-- Scaling of war heroes manpower lost to war support impact, will be added weekly as a war support penalty
	MAX_HEROES_BEING_KILLED_WEEKLY_WAR_SUPPORT_PENALTY = -0.000,		-- Max penalty that will gained per week from war heroes manpower lost
	HEROES_BEING_KILLED_WEEKLY_WAR_SUPPORT_PENALTY_DECAY = 0.0000,	-- Weekly decay of war heroes manpower lost war support penalty
	MAX_HEROES_BEING_KILLED_WAR_SUPPORT_IMPACT = -0.0,				-- Max total penalty from war heroes manpower lost
	WAR_SUPPORT_FROM_CASUALTIES = 0.000,							-- Base value (inverted) for calculating heroes being killed

	CONVOYS_BEING_RAIDED_WAR_SUPPORT_PENALTY_SCALE = -0.00,			-- Scaling of trade convoy raided to war support impact, will be added weekly as a war support penalty
	MAX_CONVOYS_BEING_RAIDED_WEEKLY_WAR_SUPPORT_PENALTY = -0.000,	-- Max penalty that will gained per week from trade convoy raided
	CONVOYS_BEING_RAIDED_WEEKLY_WAR_SUPPORT_PENALTY_DECAY = 0.000,	-- Weekly decay of trade convoy raided war support penalty
	MAX_CONVOYS_BEING_RAIDED_WAR_SUPPORT_IMPACT = -0.0,				-- Max total penalty from trade convoy raided
	...
	CONVOYS_SUNK_MULTIPLIER_FOR_WAR_SUPPORT = 0.0,	-- once a trade convoy ship sunk, you will get a larger negative impact on your war support
	CONVOYS_BEING_RAIDED_DAILY_WAR_SUPPORT_IMPACT_FROM_OVERSEA_STATES = 0.0,	-- resource transfer convoys convoys from our states being raided will give a daily war support penalty depending on how important that resource is and how inefficent convoys are
	CONVOYS_SUNK_MULTIPLIER_FOR_WAR_SUPPORT_FROM_OVERSEA_STATES = 0.0,			-- once a resource transfer convoys from our states ship sunk, you will get a larger negative impact on your war support
	CONVOYS_BEING_RAIDED_DAILY_WAR_SUPPORT_IMPACT = 0.0,	-- trade convoys being raided will give a daily war support penalty depending on how important that resource is and how inefficent convoys are

	MAX_PROPAGANDA_STABILITY_IMPACT = -0.4,			-- Max total penalty from operative performing the propaganda mission in a country
	MAX_PROPAGANDA_WAR_SUPPORT_IMPACT = -0.4,		-- Max total penalty from operative performing the propaganda mission in a country
	PROPAGANDA_STABILITY_DAILY_DECAY = 0.1,		-- Amount of stability recovered daily from propaganda effort
	PROPAGANDA_WAR_SUPPORT_DAILY_DECAY = 0.1,		-- Amount of war support recovered daily from war support effort

	NUM_DAYS_TO_FULLY_DELETE_STOCKPILED_EQUIPMENT = 10,	 -- time in days to fully delete equipments from stockpile. when you delete an equipment, they go to a temporary hidden pool which still can be seized
	AIR_SUPPLY_CONVERSION_SCALE = 0.02,				-- Conversion scale for planes to air supply
	AIR_SUPPLY_DROP_EXPIRATION_HOURS = 1,			-- Air drop length after being dropped
	...
	SPECIAL_FORCES_CAP_MIN = 6,					-- You can have a minimum of this many special forces battalions, regardless of the number of non-special forces battalions you have, this can also be modified by a country modifier
	DAYS_OF_WAR_BEFORE_SURRENDER = 1,				-- Number of days a war has to have existed before anyone can surrender in it

	FUEL_LEASE_CONVOY_RATIO = 0.5,				-- num convoys needed per fuel land lease
...
NResistance = {
	...
	RESISTANCE_COOLDOWN_WHEN_DISABLED = 0.0,				-- resistance cooldown when the state is taken back by its original owner (compliance is zeroed in that case)
...
MIN_DAMAGE_TO_GARRISONS_MODIFIER = 0.0, -- modifier that applies to losses from resistance attack to garrisons at most can be reduced to this amount
	...
	RESISTANCE_GROWTH_BASE = 0.0, -- base resistance grow
	RESISTANCE_GROWTH_MIN = 0.0, -- min resistance grow
	RESISTANCE_GROWTH_MAX = 0.0, -- max resistance grow
	...
	COMPLIANCE_DECAY_PER_EXILE_LEGITIMACY = -0.0, -- higher legitimacy will give higher decay to compliance

	RESISTANCE_RATIO_DIFF_TO_SPREAD = 0.0, -- resistance diff between two neighbour states will spread by this ratio ( from highest resistance states to lower ones and it will only spread once to a state)

	RESISTANCE_ACTIVITY_CHANCE_AT_MAX_RESISTANCE = 0.0,
	RESISTANCE_ACTIVITY_MIN_GARRISON_PENETRATE_CHANCE = 0.0,
	...
	NO_COMPLIANCE_GAIN_ENABLE_LIMIT = 0.0, -- at least this ratio of no garrison law should be active in order to no compliance gain to take effect

	GARRISON_MANPOWER_MIN_DELIVERY_SPEED = 0.0,	-- Minimum base delivery speed if the chunk can't be calculated.
	GARRISON_MANPOWER_REINFORCEMENT_SPEED = 0.0,	-- Modifier for garrison manpower reinforcement.  This value is the maximum to be delivered which is then modified by distance
	GARRISON_EQUIPMENT_DELIVERY_SPEED = 0.0,	-- Modifier for garrison equipment reinforcement speed

	GARRISON_STR_POW_MANPOWER = 0,	--Scales impact of manpower deficiency by raising that deficiency to the number here. Formula: efficiency = 1.0 - manpower_deficiency^GARRISON_STR_POW_MANPOWER
	GARRISON_STR_POW_EQUIPMENT = 0,	--Scales impact of euqipment deficiency by raising that deficiency to the number here. Formula: efficiency = 1.0 - equipment_deficiency^GARRISON_STR_POW_EQUIPMENT

	SUPPRESSION_NEEDED_BY_RESISTANCE_POINT = 0.0, -- Number of suppression point we need for each 1% of resistance
	SUPPRESSION_NEEDED_LOWER_CAP = 0.0,	-- if resistance is lower than this value then we always act as though it is at the define for the purpose of suppresion requirements
	SUPPRESSION_NEEDED_UPPER_CAP = 0.0, -- if resistance is greater than this value then we always act as though it is at the define for the purpose of suppresion requirements
	...
	MAXIMUM_GARRISON_HARDNESS_WHEN_ATTACKED = 0.0,   -- Cap to be sure that garrison will suffer lost in attack, even with a almost 100% hardness

	FOREIGN_MANPOWER_MIN_THRESHOLD = 0,		 -- The minimum number of Manpower that AI will accept to give at once, in order to avoid many weird little transfer.
	MANPOWER_BUFFER_TO_NOT_GIVE_MINOR = 0.0, -- To determine how much AI can give as foreign manpower, we calculate how much manpower we use, and add this buffer. The result is what we want to keep, for minor countries. So higher this number is, lower we will give Manpower.
	MANPOWER_BUFFER_TO_NOT_GIVE_MAJOR = 0.0, -- To determine how much AI can give as foreign manpower, we calculate how much manpower we use, and add this buffer. The result is what we want to keep, for major countries. So higher this number is, lower we will give Manpower.
	MAX_GARRISON_RATIO_WE_AGREE_TO_SUPPORT = 0.0,	-- The part of the manpower needed by the foreign garrison, that AI will agree to support with our manpower. If negative number, AI will not take into consideration the need, and just calculate how much they can give.
	FOREIGN_MANPOWER_AI_COOLDOWN_DAYS = 0,		 -- Number of days after an AI give us manpower before the AI accept to give more.
...
NProduction = {
	...
	MAX_CIV_FACTORIES_PER_LINE = 5,	-- Max number of factories that can be assigned a single production line.
	DEFAULT_MAX_NAV_FACTORIES_PER_LINE = 0,
	FLOATING_HARBOR_MAX_NAV_FACTORIES_PER_LINE = 0,
	CONVOY_MAX_NAV_FACTORIES_PER_LINE = 0,
	CAPITAL_SHIP_MAX_NAV_FACTORIES_PER_LINE = 0,
	MAX_MIL_FACTORIES_PER_LINE = 5,
	RAILWAY_GUN_MAX_MIL_FACTORIES_PER_LINE = 5,
	RAILWAY_GUN_REPAIR_SPEED = 10.0,			-- Railway gun strength repair speed per factory
	EFFICIENCY_LOSS_PER_UNUSED_DAY = 0,		-- Daily loss of efficiency for unused factory slots ( efficiency is tracked per factory slot in the production line )
	...
	LEND_LEASE_DELIVERY_TOTAL_DAYS = 1,                    -- Nr of days between lend lease deliveries
	...
	MIN_POSSIBLE_TRAINING_MANPOWER = 6,	-- How many deployment lines minimum can be training
...
NTechnology = {
	...
	BASE_TECH_COST = 10,					-- Base cost for a tech. multiplied with tech cost and ahead of time penalties
...
NMilitary = {
	...
	CORPS_COMMANDER_DIVISIONS_CAP = 6,			-- how many divisions a corps commander is limited to. 0 = inf, < 0 = blocked
	...
	MAX_DIVISION_BRIGADE_WIDTH = 1,			-- Max width of regiments in division designer.
...
NFocus = {
	...
	MAX_SAVED_FOCUS_PROGRESS = 1,				-- This much progress can be saved while not having a focus selected
},
